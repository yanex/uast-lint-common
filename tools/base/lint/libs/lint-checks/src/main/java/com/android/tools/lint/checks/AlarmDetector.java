...
public class AlarmDetector extends Detector implements Detector.UastScanner {
    ...
    public void visitFunctionCallExpression(@NonNull JavaContext context,
        ...
        if (function.matchesNameWithContaining("android.app.AlarmManager", "setRepeating")
                && function.getValueParameterCount() == 4) {
            ensureAtLeast(context, call, 1, 5000L);
            ensureAtLeast(context, call, 2, 60000L);
        }
    }
    ...
    private static long getLongValue(JavaContext context, @NonNull UExpression argument) {
