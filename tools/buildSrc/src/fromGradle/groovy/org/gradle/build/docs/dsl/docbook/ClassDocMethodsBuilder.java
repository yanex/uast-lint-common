    ...
    public void build(ClassDoc classDoc) {
        ...
        for (Element tr : children(classDoc.getMethodsTable(), "tr")) {
            ...
            for (MethodMetaData method : methods) {
                ...
                if (property != null && method.getParameters().size() == 1
                        && (method.getParameters().get(0).getType().getSignature().equals(Closure.class.getName())
                        // TODO: Remove special case for Android plugin?
                        || method.getParameters().get(0).getType().getName().equals("org.gradle.api.Action"))) {
